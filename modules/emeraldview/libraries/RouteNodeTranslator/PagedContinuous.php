 ...
 */
/**
 * RouteNodeTranslator_PagedContinuous finds a cousin for a given paged
 * Node_Document from a continuously paginated Collection based on route
 * arguments
 *
 * @package libraries
 ...
 */
class RouteNodeTranslator_PagedContinuous extends RouteNodeTranslator
{
  /**
  ...
  {
    if ( ! isset( $subnode_args[0] ) ) {
      return $this->root_node->getCousin( 1 );
    }

    if ( is_string( $subnode_args[0] ) || is_int( $subnode_args[0] ) ) {
      // FIXME: not yet implemented
      return $this->root_node->getContinuousCousinByTitle( $subnode_args[0] );
    }
    else {
