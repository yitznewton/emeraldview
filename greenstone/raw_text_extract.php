 ...
 */
if ( $argc != 3 ) {
  echo 'Usage: raw_text_extract.php build_directory index_sub_directory' . chr(10);
  exit(1);
}
...
while ( $line = fgets( STDIN ) ) {
  if ( substr( $line, 0, 4 ) == '<Doc' ) {
    ...
    $xml_blob = '<?xml version="1.0" encoding="UTF-8"?>' . chr(10);
  }
...
function process_xml_blob( $blob, $directory )
{
  ...
  foreach ( $sections as $section ) {
    extract_section( $section, $directory );
  }
}

function extract_section( DOMNode $dom_node, $directory )
{
